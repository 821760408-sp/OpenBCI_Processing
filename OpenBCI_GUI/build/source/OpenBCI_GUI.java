...
//    }
  ...
  for (int Ichan=0;Ichan < nchan; Ichan++) data_elec_imp_ohm[Ichan] = (sqrt(2.0f)*eegProcessing.data_std_uV[Ichan]*1.0e-6f) / openBCI.get_leadOffDrive_amps();
}
...
class EEG_Processing_User {
  ...
  float channel1Average = 0.0f;   //this will change over time ... used for calculations below


  float lastAve = 0.0f; // store the last average value
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    myAverage = myAverage / PApplet.parseFloat(averagePeriod); //finishing the average
...
//    }
    ...
    channel1Average = channel1Average / PApplet.parseFloat(averagePeriod); //finishing the average
    ...
    switch (guiPage) {  //the rest of the elements depend upon what GUI page we're on
      ...
      case GUI_PAGE_HEADPLOT_SETUP:
        ...
        if (drawSampleBlock) {
          ...
          int h = height - PApplet.parseInt(helpWidget.h);
          //pushStyle();
          //fill(boxColor);
          //strokeWeight(1);
          //stroke(boxStrokeColor);
